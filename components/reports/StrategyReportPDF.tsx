...
const CoverPage: React.FC<{ reportData: ReportData; clientName?: string; userName?: string }> = ({ reportData, clientName, userName }) => (
  <Page size="A4" style={styles.page}>
    <View style={styles.coverPage}>
      ...
      <Text style={styles.coverSubtitle}>
        {clientName || reportData.clientName || 'Client'} Content Library Analysis
      </Text>
      <Text style={styles.coverDate}>
        Account: {clientName || reportData.clientName || 'Client'}
      </Text>
      <Text style={styles.coverDate}>
    ...
    <View style={styles.footerContainer}>
      ...
      />
      <Text>
        {clientName || reportData.clientName ? `${clientName || reportData.clientName} • ` : ''}
        {new Date(reportData.generatedAt).toLocaleDateString('en-US', {
          year: 'numeric',
          month: 'short',
          day: 'numeric',
        })}
        {userName ? ` • Produced by: ${userName}` : ''}
        {' • Generated by Nytro.ai'}
      </Text>
    </View>
  </Page>
);
...
// Assets by ICP Page Component
const AssetsByICPPage: React.FC<{ reportData: ReportData }> = ({ reportData }) => {
  const icpEntries = Object.entries(reportData.assetDetailsByICP || {});
  ...
  return (
    <Page size="A4" style={styles.page}>
      <View style={styles.section}>
        <Text style={styles.sectionTitle}>Asset Inventory - By ICP Target</Text>

        <Text style={{ marginBottom: 15, fontSize: 12, color: colors.darkGray }}>
          Complete listing of all assets organized by Ideal Customer Profile (ICP) target, including Title, Function, Seniority, and Pain Clusters
        </Text>

        {icpEntries.map(([icpTarget, assets]) => (
          <View key={icpTarget} style={{ marginBottom: 25 }}>
            <Text style={styles.subsectionTitle}>{icpTarget}</Text>
            <View style={styles.table}>
              <View style={styles.tableRow}>
                <Text style={[styles.tableCell, styles.tableCellHeader, { flex: 3 }]}>Title</Text>
                <Text style={[styles.tableCell, styles.tableCellHeader, { flex: 2 }]}>Function</Text>
                <Text style={[styles.tableCell, styles.tableCellHeader, { flex: 1.5 }]}>Seniority</Text>
                <Text style={[styles.tableCell, styles.tableCellHeader, { flex: 2 }]}>Pain Clusters</Text>
              </View>
              {assets.map((asset, index) => (
                <View key={index} style={styles.tableRow}>
                  <Text style={[styles.tableCell, { flex: 3 }]}>{asset.title}</Text>
                  <Text style={[styles.tableCell, { flex: 2 }]}>{asset.function || '—'}</Text>
                  <Text style={[styles.tableCell, { flex: 1.5 }]}>{asset.seniority || '—'}</Text>
                  <Text style={[styles.tableCell, { flex: 2 }]}>{asset.painClusters.length > 0 ? asset.painClusters.join(', ') : '—'}</Text>
                </View>
              ))}
            </View>
          </View>
...
// Assets by Pain Cluster Page Component
const AssetsByPainClusterPage: React.FC<{ reportData: ReportData }> = ({ reportData }) => {
  const painClusterEntries = Object.entries(reportData.assetDetailsByPainCluster || {});
  
  if (painClusterEntries.length === 0) {
    return null;
  }
  ...
  return (
    <Page size="A4" style={styles.page}>
      <View style={styles.section}>
        <Text style={styles.sectionTitle}>Asset Inventory - By Pain Cluster</Text>

        <Text style={{ marginBottom: 15, fontSize: 12, color: colors.darkGray }}>
          Complete listing of all assets organized by Pain Cluster, including Title, Function, Seniority, and ICP Targets
        </Text>

        {painClusterEntries.map(([painCluster, assets]) => (
          <View key={painCluster} style={{ marginBottom: 25 }}>
            <Text style={styles.subsectionTitle}>{painCluster}</Text>
            <View style={styles.table}>
              <View style={styles.tableRow}>
                <Text style={[styles.tableCell, styles.tableCellHeader, { flex: 3 }]}>Title</Text>
                <Text style={[styles.tableCell, styles.tableCellHeader, { flex: 2 }]}>Function</Text>
                <Text style={[styles.tableCell, styles.tableCellHeader, { flex: 1.5 }]}>Seniority</Text>
                <Text style={[styles.tableCell, styles.tableCellHeader, { flex: 2 }]}>ICP Targets</Text>
              </View>
              {assets.map((asset, index) => (
                <View key={index} style={styles.tableRow}>
                  <Text style={[styles.tableCell, { flex: 3 }]}>{asset.title}</Text>
                  <Text style={[styles.tableCell, { flex: 2 }]}>{asset.function || '—'}</Text>
                  <Text style={[styles.tableCell, { flex: 1.5 }]}>{asset.seniority || '—'}</Text>
                  <Text style={[styles.tableCell, { flex: 2 }]}>{asset.icpTargets.length > 0 ? asset.icpTargets.join(', ') : '—'}</Text>
                </View>
              ))}
            </View>
          </View>
...
const StrategyReportPDF: React.FC<StrategyReportPDFProps> = ({ reportData, clientName, brandContext, userName }) => (
  <Document>
    <CoverPage reportData={reportData} clientName={clientName} userName={userName} />
    ...
    {Object.keys(reportData.assetDetailsByICP || {}).length > 0 && (
      <AssetsByICPPage reportData={reportData} />
    )}
    {Object.keys(reportData.assetDetailsByPainCluster || {}).length > 0 && (
      <AssetsByPainClusterPage reportData={reportData} />
    )}
